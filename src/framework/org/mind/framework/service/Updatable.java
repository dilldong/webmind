...
public interface Updatable {
    ...
    static boolean isReplica() {
        String replica = getRunArgs(Service.SVC_REPLICA_NAME_ENV);
        if(StringUtils.isNotBlank(replica))
            return BooleanUtils.toBoolean(replica);

        return BooleanUtils.toBoolean(getEnvArgs(Service.SVC_REPLICA_NAME, BooleanUtils.FALSE));
    }
    ...
    static String getAppInstanceId() {
        String instanceId = getRunArgs(Service.APP_INSTANCE_ID_ENV);
